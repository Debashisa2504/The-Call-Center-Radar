...
Team: Ghost Hunters — Debashisa Behera · Aakansha Sharma Call-Centre Analytics Hackathon 2026 · 
...
The transcript analysis says the agent claimed the issue was resolved (transcript_resolved = true)
The same customer calls back within 30 minutes (ghost_resolved = true)
...
Intent-aware matching: only same-topic callbacks count as a ghost. A callback about a different issue is treated as a new call, not a failure.
...
1. Transcription	pipeline/transcribe.py	ffmpeg splits each stereo MP3 into two mono channels in one pass (agent = left, customer = right). Each channel is sent to Whisper independently — no diarization needed. For mono recordings (customer channel RMS < 0.01), the system transcribes once and assigns speakers by alternating on pause gaps > 1s.
...
4. Trend clustering	pipeline/trends.py	Each call's intent string is embedded (text-embedding-3-large) and clustered with k-means (k=10 by default); each cluster is labelled by GPT-4o and surfaced in the Trends tab.
...
Pre-filter — embed the rule; retrieve top-K semantically similar transcript chunks via cosine similarity, re-ranked by a cross-encoder. Keyword hits bypass the similarity floor.
LLM judge — GPT-4o is shown only those chunks and asked whether the rule was violated, with a required citation (speaker, timestamp, exact quote).
Verifier — the cited quote is checked as a verbatim substring of the transcript at the cited timestamp. If it doesn't match, the violation is marked unverified and not surfaced as fact.

Five of the 14 rules (re-asked information, missing closing question, missing escalation offer, missing fraud disclosure, missing frustration acknowledgement) are deterministic keyword/pattern checks rather than LLM calls — these proved more reliable and cheaper than asking GPT to detect an absence of something.

Rule evaluation writes atomically: results are DELETE + INSERT'd in a single transaction after all 14 rules finish, so a mid-evaluation crash never leaves partial results, and old results stay visible until fresh ones are ready.
...
AI	Azure OpenAI — gpt-4o (analysis, compliance judge, cluster labelling), text-embedding-3-large (embeddings), whisper (transcription)
...
Azure OpenAI account with gpt-4o, text-embedding-3-large, and whisper deployments
...
# Azure OpenAI (takes precedence) — or use standard OpenAI
AZURE_OPENAI_ENDPOINT=https://your-resource.openai.azure.com/
OPENAI_API_KEY=<your-key>
AZURE_OPENAI_API_VERSION=2024-12-01-preview

# Model deployment names (match your Azure deployment names exactly)
ANALYSIS_MODEL=gpt-4o
EMBED_MODEL=text-embedding-3-large
WHISPER_MODEL=whisper

# Optional tuning
GHOST_WINDOW_MIN=30     # minutes before a callback counts as a ghost
INGEST_WORKERS=2        # parallel ingest workers
EOF
...
Helper scripts at the project root (show_rules.py, check_call.py, find_bad_calls.py, batch_compliance.py) were built during development for inspecting the database and re-running specific pipeline stages without a full re-ingest — see inline comments in each for usage.

9. Engineering Notes & Bugs Fixed
Compliance engine
Rewrote 5 of the 14 rules (re-asked info, missing closing question, missing escalation offer, missing fraud disclosure, missing frustration acknowledgement) from LLM calls to deterministic keyword/pattern checks — these ask the model to detect an absence, which GPT handled unreliably.
Made rule writes atomic (DELETE + INSERT in a single transaction, only after all 14 rules finish) so a crash mid-evaluation can't leave partial results, and old results stay visible until fresh ones are ready.
Fixed evidence-quote accuracy so the cited quote is the specific matching phrase within a chunk, not the whole chunk.
Added asyncio.Semaphore(1) to prevent concurrent evaluation requests from crashing the process.
Added batch_compliance.py to evaluate all calls deterministically in ~30 seconds with zero GPT calls.
Ghost detection
...
Rebuilt the spam-callers view: intents are now bucketed into 17 canonical topics so repeat-rate is measured correctly; the view flags customers repeatedly calling about self-serviceable issues (balance checks, branch hours, password resets).
Fixed a metric that was undercounting positive outcomes (previously counted only explicitly "happy" calls; corrected to count anything that isn't frustrated/concerned/angry/anxious).
Fixed a %%-escaping bug in dynamically built LIKE queries and a blank-row bug in the Trends table caused by null trend labels.
Other fixes
...
Medium	Noisy transcripts	Background TV/audio sometimes gets transcribed by Whisper — voice-activity-detection pre-processing would help
...
Low	seed_default_rules runs on every startup	Harmless (DO NOTHING prevents duplicates) but unnecessary I/O on every boot
